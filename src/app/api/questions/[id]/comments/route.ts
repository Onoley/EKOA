...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
 ...
 if(error){console.warn("comments.query_failed",{code:error.code});return NextResponse.json({message:"Impossible de charger les commentaires."},{status:403})}const parsed=z.array(commentSchema).safeParse(data??[]);if(!parsed.success){console.warn("comments.response_invalid",parsed.error.issues.map(({code,path})=>({code,path})));return NextResponse.json({message:"Réponse invalide."},{status:503})}
