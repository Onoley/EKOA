...
export default async function CreatePage({ searchParams }: { searchParams: Promise<{ draft?: string; wave?: string }> }) {
  ...
  return <main className="create-page min-h-[calc(100dvh-4.75rem)] p-5"><header className="create-heading"><p>Nouvelle contribution</p><h1>{params.wave ? "Nouvelle vague" : params.draft ? "Modifier le brouillon" : "Créer une question"}</h1><span>Posez librement une question courte à la communauté.</span></header>{error || tagError ? <div role="alert" className="error-state">Impossible de charger les catégories et leurs tags.</div> : categoryRows.length ? <QuestionForm categories={categoryRows} tagsByCategory={tagsByCategory} initial={initial} /> : <div className="empty-state">Aucune catégorie disponible.</div>}</main>;
}
