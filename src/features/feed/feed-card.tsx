...
export function FeedCard({ item, feed, rank, requestId, algorithmVersion,initialResults }: Props) {
  ...
  return <div ref={element} className="h-[calc(100dvh-4.75rem)] snap-start snap-always">
    <QuestionCard questionId={item.question_id} question={item.question_text} category={item.category_name} author={item.author_username} verified={item.author_verified} options={item.options} initialResults={initialResults} initiallyFollowed={item.initially_followed} initiallyUpvoted={item.initially_upvoted} initialUpvoteCount={item.upvote_count} sponsoredBy={item.sponsored_by} headingLevel="h2" immersive />
  </div>;
}
