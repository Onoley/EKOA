...
export const feedItemSchema = candidateSchema.pick({ question_id: true, question_text: true, author_id: true, author_username: true, author_verified: true, category_id: true, category_name: true, published_at: true, options: true, upvote_count: true, initially_followed: true }).extend({ initially_upvoted: z.boolean(), sponsored_by:z.string().min(2).max(160).nullable() });
