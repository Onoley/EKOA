...
type Props = { questionId: string; question: string; category: string; authorId: string; author: string; verified: boolean; options: Option[]; initialResults?: ResultRow[]; initiallyFollowed: boolean; initiallyUpvoted?: boolean; initialUpvoteCount?: number; headingLevel?: "h1" | "h2"; showComments?: boolean; sponsoredBy?:string|null; immersive?:boolean };
...
export function QuestionCard(props: Props) {
  ...
  return <article className={props.immersive?"feed-question-card":"rounded-3xl border border-black/10 bg-white p-5 shadow-sm"} aria-labelledby={`question-${props.questionId}`}>
    ...
    <div className={props.immersive?"feed-action-rail":"mt-5 grid grid-cols-2 gap-2 border-t border-black/10 pt-4 sm:grid-cols-3"} aria-label="Actions de la question">
      {props.immersive?(props.author === "membre supprimé"?<ProfileAvatar userId={props.authorId} username={props.author} supabaseUrl={supabaseUrl} className="feed-author-avatar" />:<Link href={`/profils/${props.author}`} className="feed-author-avatar-link" aria-label={`Voir le profil de ${props.author}`}><ProfileAvatar userId={props.authorId} username={props.author} supabaseUrl={supabaseUrl} className="feed-author-avatar" verified={props.verified}/></Link>):null}
      {props.immersive?<ReportForm targetType="question" targetId={props.questionId} label="Signaler cette question" compact />:null}
