...
export function QuestionCard(props: Props) {
  ...
  return <article className={props.immersive?"feed-question-card":"rounded-3xl border border-black/10 bg-white p-5 shadow-sm"} aria-labelledby={`question-${props.questionId}`}>
    {props.sponsoredBy?<p className="mb-4 rounded-2xl border border-black bg-[var(--accent)] px-4 py-3 text-sm font-bold">Question sponsorisée par {props.sponsoredBy}</p>:props.adminFeatured?<p className="admin-featured-label">Sélection Ekoa</p>:null}
