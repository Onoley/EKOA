...
type Props = { questionId: string; question: string; category: string; author: string; verified: boolean; options: Option[]; initialResults?: ResultRow[]; initiallyFollowed: boolean; initiallyUpvoted?: boolean; initialUpvoteCount?: number; headingLevel?: "h1" | "h2"; showComments?: boolean; sponsoredBy?:string|null; immersive?:boolean };
...
export function QuestionCard(props: Props) {
  ...
  return <article className={props.immersive?"feed-question-card":"rounded-3xl border border-black/10 bg-white p-5 shadow-sm"} aria-labelledby={`question-${props.questionId}`}>
    ...
    <div className={props.immersive?"feed-question-copy":""}><div><p className="eyebrow">{props.category}</p><p className="mt-3 text-sm font-semibold text-[var(--muted)]">{props.author === "membre supprimé" ? "Membre supprimé" : <Link href={`/profils/${props.author}`} className="underline">@{props.author}</Link>}{props.verified ? " · ✓ Vérifié" : ""}</p></div>
    ...
    {follow.message ? <p role={follow.status === "error" ? "alert" : "status"} className="mt-3 text-sm text-[var(--muted)]">{follow.message}</p> : null}
    ...
    <div className={props.immersive?"feed-report":""}><ReportForm targetType="question" targetId={props.questionId} label="Signaler cette question" /></div>
