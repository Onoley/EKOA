...
export function AvatarForm({ userId, username, supabaseUrl }: { userId: string; username: string; supabaseUrl: string }) {
  ...
  return <section className="mt-6 rounded-3xl border border-[var(--border)] p-5" aria-labelledby="avatar-title">
    ...
    <div className="mt-4 flex items-center gap-4">
      ...
      <p className="text-sm leading-5 text-[var(--muted)]">JPEG, PNG ou WebP. 5 Mo maximum.</p>
    </div>
    <input ref={libraryInput} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={selectFile} />
    <input ref={cameraInput} type="file" accept="image/jpeg,image/png,image/webp" capture="user" className="sr-only" onChange={selectFile} />
